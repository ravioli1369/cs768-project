...
CPF_data = ["cora", "citeseer", "pubmed", "a-computer", "a-photo", "wikipedia", "mooc"]
...
def get_train_val_test_split(
    random_state, labels, train_examples_per_class, val_examples_per_class
):
    """Create train/val/test split for semi-supervised node classification.

    Parameters
    ----------
    random_state : numpy.random.RandomState
        Random number generator
    labels : array-like [num_samples, num_classes]
        Binary indicator matrix where second dimension is number of classes
    train_examples_per_class : int
        Number of examples per class for training
    val_examples_per_class : int
        Number of examples per class for validation
    ...
    # Find classes with at least one example
    valid_classes = []
    for class_index in range(num_classes):
        if np.sum(labels[:, class_index]) > 0:
            valid_classes.append(class_index)
    
    # If no valid classes, create dummy splits
    if len(valid_classes) == 0:
        ...
        print("Warning: No valid classes found. Using random train/val/test split.")
        return train_indices, val_indices, test_indices
    
    # Use only valid classes for sampling
    train_indices = sample_per_class(random_state, labels, train_examples_per_class)
    # Makes sure that validation indices are different from training indices
    val_indices = sample_per_class(
        random_state, labels, val_examples_per_class, forbidden_indices=train_indices
    )
    
    # Get test indices
    test_indices = np.setdiff1d(np.arange(num_samples), np.concatenate((train_indices, val_indices)))
